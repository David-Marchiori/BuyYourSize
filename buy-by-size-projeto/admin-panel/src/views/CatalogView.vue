...
import { 
  ...
  MoreHorizontal, ArrowRight, Package, AlertCircle, CheckCircle2, X
...
const syncUrl = ref('https://village23.com.br/wp-content/uploads/feed/facebook/xml/feedpadrao.xml'); // Valor padrão
...
const handleSyncCatalog = async () => {
  ...
  try {
    ...
    setTimeout(() => { showSyncModal.value = false; syncMessage.value = ''; }, 2000); // Fecha modal após sucesso
  } catch (err) {
...
const filteredProducts = computed(() => {
  ...
  return products.value.filter(p => 
    p.nome_regra.toLowerCase().includes(lowerTerm) || 
    p.produto_id.toLowerCase().includes(lowerTerm)
  );
});
    ...
    <transition name="fade">
      <div v-if="showSyncModal" class="sync-overlay" @click.self="showSyncModal = false">
        <div class="sync-card animate-scale">
          ...
          <div class="sync-body">
            <label>URL do Feed XML (Google/Facebook)</label>
            <div class="input-wrapper">
              <input type="url" v-model="syncUrl" placeholder="https://loja.com/feed.xml" :disabled="syncLoading">
            </div>
...
.btn-text {
  background: none; border: none; color: #64748b; font-weight: 500; cursor: pointer; padding: 10px 20px;
}

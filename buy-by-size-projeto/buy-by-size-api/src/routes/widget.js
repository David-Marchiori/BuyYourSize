...
const { evaluateCondition, parsePhraseList } = require("../utils/rules");
...
router.post("/sugestao", async (req, res) => {
  ...
  try {
    ...
    if (!produto.modelagem_id) {
      return res
        .status(200)
        .json({ sugestao: null, message: "Sem tabela vinculada." });
    }
    ...
    if (tipoTabela === "calcado") {
      ...
      for (const regra of regras) {
        if (
          peCliente >= parseFloat(regra.pe_min) &&
          peCliente <= parseFloat(regra.pe_max)
        ) {
          sugestaoEncontrada = {
    ...
    if (sugestaoEncontrada) {
      ...
      res
        .status(200)
        .json({ sugestao: null, message: "Nenhuma regra encontrada." });
    }
  } catch (error) {
